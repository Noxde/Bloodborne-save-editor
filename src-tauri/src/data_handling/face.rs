...
pub fn export(file_data: &FileData, path: &str) -> Result<(), Error> {
    ...
    let mut buffer = [0; 16];
    for i in (file_data.offsets.face .. file_data.bytes.len() - 15).step_by(16) {
    ...
    if face_end == 0 {
       return Err(Error::CustomError("Failed to find the end of the face."));
    }

    let mut export_bytes = Vec::new();
    export_bytes.extend_from_slice(&file_data.bytes[file_data.offsets.face ..= face_end]);
...
mod tests {
    ...
    fn test_export() {
        ...
        let start = file_data.offsets.face;
        let end = [0, 0, 0, 0, 0xFF, 0xFF, 0xFF, 0xFF, 0, 0, 0, 0, 0, 0, 0, 0];
